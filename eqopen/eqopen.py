...
def eqbacktests(eq_credentials_path: str = 'EQCredentials', branch: str = 'main', *argc, **argv) -> 'EQBacktests':
    ...
    # Install Library
    subprocess.check_call(['pip', 'install', f"git+https://{eq_credentials['GitHub_Username']}:{eq_credentials['GitHub_Token']}@github.com/exp-Quant/EQBacktests@{branch}"], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
